...
interface MaMiniCardsProps {
  cards: MaMiniCardData[];
  metric: MaMetricType;
 ...
 * Each card shows entity name, mini chart, latest values, and GAP.
 */
export default function MaMiniCards({
  cards,
  metric,
  selectedEntityId,
  onEntitySelect,
...
/** Tooltip shared by all mini cards — receives metric context via closure props. */
function MiniTooltipContent({
  ...
  if (ma?.value != null) {
    items.push({
      color: "#64748B",
      label: "MA",
      value: fmtVal(ma.value as number),
    });
  }
...
const MiniCard = memo(function MiniCard({
  ...
  return (
    ...
    >
      {/* Entity name */}
      <div className="mb-1.5 truncate text-xs font-semibold text-gray-700" title={card.entityName}>
      ...
      <div className="mt-1.5 space-y-0.5">
        ...
        <div className="flex items-center justify-between text-[10px]">
          <span className="text-gray-400">
            MA: {card.latestMa != null ? fmtVal(card.latestMa) : "-"}
          </span>
          <span className={cn("font-semibold", gapColorClass(card.latestGap))}>
            {formatGapPct(card.latestGap)}
          </span>
        </div>
      </div>
    </button>
